...
const Update = () => {
  ...
  // تحميل المنشور المطلوب تعديله
  useEffect(() => {
    // API لجلب بيانات المنشور
    fetch(`http://127.0.0.1:8000/api/posts/${id}`)
      .then(response => response.json())
      .then(post => {
        // تعبئة النموذج بالبيانات الحالية للمنشور
  ...
  const onSubmitForm = async (data) => {
    ...
    if (file) {
      formData.append('image', file); // إضافة الصورة إذا تم رفع صورة جديدة
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate('/'); // استخدام navigate للرجوع إلى الصفحة الرئيسية بعد التعديل
      } else {
  ...
  return (
    <div className="container mx-auto py-10">
      <h2 className="text-3xl mb-6 text-center">Edit Post</h2>
